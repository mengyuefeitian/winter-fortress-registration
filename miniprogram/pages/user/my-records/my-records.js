// pages/user/my-records/my-records.js
// 「我的报名」独立页面：从「我的」个人中心拆出，只承载报名记录的查看与取消
const app = getApp()
...
Page({
  ...
  // 等待角色就绪（navigateTo 进入时通常已就绪，此处仅作兜底）
  ...
  // 缓存 key：统一按「用户 + 分区」隔离，避免切区后串数据
  cacheKey: function (userId, zoneId) {
    return 'myregs_' + userId + '_' + (zoneId || 'none')
  },

  // 当前用户 / 分区对应的缓存 key（读写共用，避免拼接不一致）
  currentCacheKey: function () {
    const userId = app.globalData.userInfo ? app.globalData.userInfo._id : app.globalData.openid
    const zoneId = app.globalData.currentZone ? app.globalData.currentZone._id : ''
    return userId ? this.cacheKey(userId, zoneId) : ''
  },

  // 缓存优先渲染：内存缓存 → 持久化缓存（TTL 5 分钟）
  applyCache: function () {
    const userId = app.globalData.userInfo ? app.globalData.userInfo._id : app.globalData.openid
    const zoneId = app.globalData.currentZone ? app.globalData.currentZone._id : ''
    if (!userId) return

    const key = this.cacheKey(userId, zoneId)
    let myCached = cache.get(key)
    if (!myCached) {
      try {
        const persisted = wx.getStorageSync('myregs_persist_' + key)
  ...
  loadMyRegistrations: async function () {
    ...
    try {
      ...
      const key = this.cacheKey(userId, currentZoneId || '')
      cache.set(key, cachePayload)
      try { wx.setStorageSync('myregs_persist_' + key, cachePayload) } catch (e) {}
  ...
  invalidateCache: function () {
    ...
    try { wx.removeStorageSync('myregs_persist_' + key) } catch (e) {}
  },
  ...
  goToRegistration: function () {
    wx.navigateTo({ url: '/pages/user/registration/registration' })
  },

  goToPositionList: function () {
    wx.navigateTo({ url: '/pages/user/position-list/position-list' })
  },

  goToArsenal: function () {
    wx.navigateTo({ url: '/pages/user/arsenal-registration/arsenal-registration' })
  },

  goToCanyon: function () {
    wx.navigateTo({ url: '/pages/user/canyon-registration/canyon-registration' })
  },

  goToBattle: function () {
    wx.navigateTo({ url: '/pages/user/battle-list/battle-list' })
  },
  ...
  onShareAppMessage: function () {
    return {
      ...
      path: '/pages/user/my-records/my-records'
    }
  }
})
